...
class TestRadixTree {
    ...
    @Test
    fun testAddingExistingValue() {
        val tree = RadixTree(StringKey("", Byte.SIZE_BITS), 0, Byte.SIZE_BITS)

        assertTrue(tree.add(StringKey("/home/jude/Documents/LGFS/.gitignore", Byte.SIZE_BITS), 0))
        assertFalse(tree.add(StringKey("/home/jude/Documents/LGFS/.gitignore", Byte.SIZE_BITS), 0))

        //assertFalse(tree.add(StringKey("home", Byte.SIZE_BITS),0))
    }
    ...
    fun testFindingPaths() {
        ...
        for (i in paths.indices) {
            assertTrue(tree.add(StringKey(paths[i], Byte.SIZE_BITS), i))
        }
        for (i in paths.indices) {
